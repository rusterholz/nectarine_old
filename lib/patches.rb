...
class Integer
  ...
  #
  # If a block is passed, the return value of the method will have an X percent chance to
  # be the result of running the block, and a reciprocal chance to be nil.
  #
  # Example:
  #   send_winning_ticket if 5.percent_chance
  #
  def percent_chance( &block )
...
class String
  ...
  # Shorthand methods for converting strings in and out of Base64.
  def to_base64
    'Base64'.constantize.encode64( self ).gsub /[\n]/, '' # by default Base64 adds obnoxious line breaks
  end
  def from_base64
    'Base64'.constantize.decode64( self )
  end
  def to_base64!
    gsub!( self, self.to_base64 )
  end
  def from_base64!
    gsub!( self, self.from_base64 )
  end
...
class Array
  ...
  # Oxford-comma-conscious join of lists. Also pays (naive) attention to the first letter of the word so
  ...
  #   %w( director actor writer ).oxford_join articles: :definite       -------> 'the director, the actor, and the writer'
  #   %w( director actor writer ).oxford_join articles: false           -------> 'director, actor, and writer'
  #   %w( director actor writer producer ).oxford_join                  -------> 'a director, an actor, a writer, and a producer'
  #
  def oxford_join( articles: :indefinite )
    prepared = map(&:to_s)
    prepared.map!{ |e| "the #{e}" } if articles == :definite
    prepared.map!{ |e| "a#{ /^[aeiou]/i.match( e ) && 'n'} #{e}" } if articles == :indefinite
